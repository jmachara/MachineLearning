...
\textbf{Answer:} (a) To constuct the tree I started by finding the total entropy by using the equation. I got .863, then calculated the entropy of each variables 2 outcomes,
 0 and 1. I used this to find the expected entropy for each variable, and then subtracted that from the entropy to get the information gain for each variable.
  I got $x1 = .062, x2 = .47, x3 = .006, and x4 = .47. $ Because x2 and x4 were tied,
I decided to used x2 for my first node. Because every path when x2 is 1 goes to 0, we can have that path go to 0, leaving us with paths 1,3, and 4. The entropy after
picking 0 for x2 is .92. I then calculated the entropy for x4 which was 0. Because the expected entropy is 0, I didn't have to calculate the other variables 
gain because when entropy is 0 the tree branch is done. I got the resulting tree for the table. 
...
\textbf{Answer:} (a) The starting ME of the tree was 5/14. I then calculated the ME for each outcome to find the gain of each variable to decide which one to split on.
I got a gain of 1/14 for outlook, 0 for temperature, 1/14 for humidity, and 0 for wind. These were calulated by multiplying the expected ME for each variable and subtracted it
from the overall ME. The outcome for the overcast branch had a ME of 0 so we can put the positive for that branch. The other two had a ME of 2/5 so we find the gain for the remaining variables for
each Outlook branch and find the highest gain to split on. For the sunny branch the gain of humidity was 2/5, which is the ME for the branch so we don't have to find the ME for the other variables since this finishes the split. 
The rainy branch had a gain of 2/5 for wind, so we can split on that variable since the that is the ME of the branch. This leaves us with the resulting tree. 
	  ...
	  child { node [env] {Humidity}
		...
		child { node [env] {wind}
				...
				child { node [env] {-}
						...
						edge from parent node [above] {rainy} };
  \end{tikzpicture}

  (b) The starting GI of the table is $45/98$. Calculating the gain of each variable with the GI gave me
  $57/490$ for outlook, $11/588$ for temperature, $9/98$ for humidity and $3/98$ for wind so I decided to split on outlook first
  because of the GI value. The GI of the sunny and rainy branches is .48. Finding of the gain of humidity resulted in .48, therefore I split on it because it fully sorts the rest of the entries.
  For the rainy branch, the gain for wind is .48 aswell so I split on that. Giving me a tree that is the exact same shape as in part a. Refer to part a for the tree picture. 

  (c) They all are the same tree because this is the most efficient way to split the data. Each way to find the gain is different but 
  finds similar things so the variables are ranked similarily in each one. 
...
\textbf{Answer:} (a) I set the unknown outlook value to sunny because it was tied for the most common with rainy. 
After calculating the gain for each variable, I got outlook at .194, temperature at .032, humidity at .17, and wind at .06. 
Therefore the best to split on is Outlook because it has the highest gain.

(b)	The most common value for outlook with a result of yes was overcast. Calculating the gain 
I got .271 for outlook, .032 for temperature, .17 for humidity, and .06 for wind. This results in a split on outlook because it has
the highest information.

(c)Using the fractional splits to calculate the gain, $5/14$ for sunny, $4/14$ for overcast, $5/14$ for rainy. I got .224 for outlook, 
.032 for temperature, .17 for humidity, and .06 for wind. This results in a split on outlook because it has the highest information gain. 

(d)After I split on Outlook, the overcast branch is finished resulting in play, on the sunny branch I caluclated the gain of humidity for the 5.357 options and got an gain of 
.99, which is the same as sunny's entropy. This means it splits the rest of the data perfectly so i didn't calculate the other gains. So after splitting on Humidity we get normal as play and high as not playing. On the rainy branch I found the gain of wind, which was .953
which is the same as the branches entropy, so i didn't calculate the other gains and split on wind. This gave me the resulting tree.  
\begin{tikzpicture}
	[
	  grow                    = right,
	...
	\node [root] {Outlook}
	  ...
	  child { node [env] {+}
			edge from parent node [above] {overcast} };
  \end{tikzpicture}
