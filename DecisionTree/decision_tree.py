...
class methods:
    ...
    def bagging_RandForest():
        ...
        attribute_array = DecTree.make_attributes_array(train_data[1])
        tree_array = []
        f = open("./Data/RandForest_Data_4_6.txt","w")
    ...
    def IntensiveBagging():
        ...
        print(str(single_tree_bias_var[0]) + " " + str(single_tree_bias_var[1]) + " " + str(single_tree_general_sqrd_err) + "\n")
        print(str(bagged_tree_bias_var[0]) + " " + str(bagged_tree_bias_var[1]) + " " + str(bagged_tree_general_sqrd_err) + "\n")
    def rand_tree_intensive_bagging():
        ...
        for i in range(1,101):
            ...
            for j in range(0,100):
                curr_data = Bagging.randData(new_data)
                tree_array.append(Bagging.RandTreeLearn(curr_data,attribute_array,train_data[1],Node(None),100,2))
            tree_array_array.append(tree_array)
        new_tree_array = []
        for TA in tree_array_array:
            new_tree_array.append(TA[0])
